...
Reed-Solomon codes, and distance from them, are the objects the FRI
protocol operates on. We recall the general definitions that the
soundness analysis of the FRI protocol relies on.
...
size for Reed-Solomon codes is controlled by the Johnson bound.
...
Relaxing the soundness of an interactive proof
(\cref{def:interactive_proof}) to hold only against efficient provers
yields a weaker but more flexible notion.
...
The BCS transformation~\cite{BCS16} compiles a public-coin IOP into a
