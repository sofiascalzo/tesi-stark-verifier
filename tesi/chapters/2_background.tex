...
\begin{example}
\label{ex:iop}
FRI is itself an IOP. In each round the prover's oracle is a committed
codeword, and the verifier's challenge is a folding coefficient; at the
end the verifier queries only a handful of positions of each oracle
rather than reading the full codewords, which is what makes
verification succinct.
\end{example}
...
An IOP of \emph{proximity} (IOP of Proximity, IOPP) is an IOP in which
the verifier
...
\begin{example}
\label{ex:pcs}
A prover commits to a polynomial $f$ by sending a single value $C$.
Later, asked for the value at a point $z$, it returns $v = f(z)$ with a
proof $\pi$. The verifier checks $\pi$ against $C$, $z$, and $v$ without
ever seeing $f$ itself; binding ensures the prover cannot open the same
$C$ to two different values at the same $z$.
\end{example}
