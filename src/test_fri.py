...
from polynomial import ntt, evaluate
...
poly = [FieldElement(c) for c in [1, 2, 3, 4, 5, 6, 7, 8]]  # grado 7, 8 coefficienti
...
proof.query_proofs[0].rounds[0].lo_value = FieldElement(999999)
...
proof.final_poly = [FieldElement(42), FieldElement(42)]
...
# ---
print("\n\nTEST 4: NTT correttezza")
 
coeffs = [FieldElement(c) for c in [3, 1, 4, 1, 5, 9, 2, 6]]
omega = two_adic_generator(3)  # radice di ordine 8
 
ntt_result = ntt(coeffs, omega)
 
for i in range(len(coeffs)):
    point = omega ** i
    direct = evaluate(coeffs, point)
    assert ntt_result[i] == direct, f"  ERRORE: NTT[{i}]={ntt_result[i]} != eval={direct}"
 
print("  NTT e valutazione diretta coincidono")
print()