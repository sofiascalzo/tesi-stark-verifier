...
def ntt(coeffs:List[int], omega: int) -> List[int]:
    ...
    w=1 # omega0
    for i in range(n//2):
        second_term = mul(w, odd[i])
        result[i] = add(even[i], second_term)
        result[i + n // 2] = sub(even[i], second_term)
        w = mul(w, omega)
...
# usato prima di LDE
def intt(evals: List[int], omega: int) -> List[int]:
...
def evaluate(coeffs: List[int], point: int) -> int:
    result = 0
    for c in reversed(coeffs):
        result = add(mul(result,point), c)
    return result
...
def coset_lde(coeffs: List[int], log_blowup: int) -> List[int]:
    n= len(coeffs)
    blowup = 1 << log_blowup
    extended_n = blowup * n
    ...
    padded = coeffs + [0] * (extended_n - n)

    extended_evaluations = [0]*extended_n
    g_power =1
    for i in range(extended_n):
        extended_evaluations[i]=mul(padded[i], g_power)
        g_power = mul(g_power, g)
